...
from modules.visualizacoes import criar_grafico_barras_verticais
...
print("\nTestando a função 'criar_grafico_barras_verticais'...")
...
try:
    ...
    caminho_amostra = os.path.join('tests', 'amostras', 'amostra_dados_contribuicao_ramo_mes.csv')
    ...
    caminho_saida = os.path.join(diretorio_atual, 'criar_grafico_barras_verticais')

    # 3. Executar a Função
    # Chama apenas a função que queremos testar, passando os dados de amostra.
    sucesso = criar_grafico_barras_verticais(df_teste, caminho_saida, "Teste do Gráfico criar_grafico_barras_verticais")
