...
def gerar_query(ano, mes):
    ...
    retornando os dados em um formato ideal para a plotagem do gráfico.
    """
    return f"""
        ...
        FROM (
            ...
            WHERE 
                ...
                AND ff.ID_CONTRIBUICAO IN (
                    ...
                    WHERE ptc.mantenedor_consolidado = 'PARTICIPANTE' AND ptc.movimentacao IN ('CONTRIBUIÇÃO', 'AJUSTE', '')
                )
            ...
            WHERE 
                ...
                AND ff.ID_CONTRIBUICAO IN (
                    SELECT ptc.ID_CONTRIBUICAO_TRUST
                    FROM portal.dbo.participante_tipo_contribuicao ptc
                    WHERE ptc.mantenedor_consolidado = 'PATROCINADOR' AND ptc.movimentacao IN ('CONTRIBUIÇÃO', 'AJUSTE', '')
                )
                AND ff.ID_CONTRIBUICAO NOT IN (491, 492)
                -- Filtro de data dinâmico
                AND ff.NR_ANO_REF = {ano} AND ff.NR_MES_REF = {mes}
    ...
    -- Ajuste para retornar em linhas ('Categoria', 'Valor'), que é o formato ideal para o gráfico
